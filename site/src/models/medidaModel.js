...
function buscarMedidasEmTempoReal(idEstufa) {
    instrucaoSql = `SELECT MAX(dht11Umidade) AS umidade, MAX(lm35Temperatura) AS temperatura, MAX(luminosidade) AS luminosidade, momento, DATE_FORMAT  (momento,'%H:%i:%s') AS momento FROM dado
    JOIN estufa ON fkEstufa = idEstufa 
    where idEstufa = ${idEstufa}
    order by idDado desc limit 7;`;
